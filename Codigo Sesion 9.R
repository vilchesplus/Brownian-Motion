...
library(readxl)
Datos <- read_excel("~/Dropbox/Master Análisis de datos/Calculo Estocastico/Datos.xls")
Datos
library(MASS)
fomega<-function(t){
  n<-length(t)
...
#9.4
x<-seq(1/253,96/253,1/253)
plot(x,valor[,],type="l",
     col="red", main="Evolución de las acciones del Banco
     Santander",
     xlab="Tiempo desde 6/8/2012 a 17/12/2012
...
#9.8
t<-seq(1/253,96/253,1/253)
plot(t,valor[,],type = "l",
     main="Predicción del valor de las acciones
     del BS en el mes próximo",
     xlab="Tiempo", ylab="Valor", col = "blue",
     xlim=c(0,(96+19)/253),ylim=c(4,7))

#9.9
pred<-prediccion(19,.10)
pred
...
tpred<-seq(96/253,(96+19)/253,1/253)
lines(tpred,inferior,col="red",lwd=2)
lines(tpred,espe,col="green",lwd=2)
lines(tpred,superior,col="red",lwd=2)

#9.11
real<-c(5.830,5.958,6.010,6.035,6.110,6.090,
        6.097, 6.008, 6.100, 6.329, 6.307, 6.307,
        6.322, 6.340, 6.401, 6.482, 6.591, 6.620,
        6.431, 6.433)
lines(tpred,real,col="blue",lwd=2)

#9.12
t<-seq(0,19/253,1/253)
a<-5.83*brownexp(mu,sigma,t)
